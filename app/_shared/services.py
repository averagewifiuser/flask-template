...
def is_in_development_environment():
    ...
    '''
    return os.getenv('ENVIRONMENT', 'nah').lower().startswith('dev')
...
def is_in_staging_environment():
    ...
    '''
    return os.getenv('ENVIRONMENT', 'nah').lower().startswith('staging')



def generate_access_token(user_id, user_type, camp_id=None, is_super=False):
    payload_data = {
        'user_id': user_id,
        'user_type': user_type,
        'is_super': is_super,
...
def set_current_user(user_type='admin', user_id=-1, school_id=-1, **kwargs):
    ...
    user_data = {'user_id': user_id, 'user_type':user_type, 'school_id': school_id, 'is_super': kwargs.get('is_super', False)}
